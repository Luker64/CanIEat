...
class ProductInfoViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        prodTotalFats.text = "Gorduras Totais: \(String(format:"%.2f", prod?.totalFat ?? ""))"
        prodFiber.text = "Fibras: \(String(format:"%.2f", prod?.fiber ?? ""))"
        prodSodium.text = "Sódio: \(String(format:"%.2f", prod?.sodium ?? ""))"
